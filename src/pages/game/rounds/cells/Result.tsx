...
const Result: React.FunctionComponent<ResultProps> = (props) => {
  ...
  if (bet?.status === "pending" && pendingWin) {
    className = "px-5 p-1 border border-grey-800 text-center bg-accent opacity-50"
  } else if (bet?.status === "pending" && !pendingWin) {
    className = "px-5 p-1 border border-grey-800 text-center bg-secondary opacity-50"
